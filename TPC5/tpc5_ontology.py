...
fileBase = '''
@prefix : <http://www.rpcw.di.uminho.pt/2025/cinema/> .
...
def writeOntologyInTTL(outFilename, movies_csv, actors_csv, characters_csv, ttl_base, ttl_last_line, ttl_url):
    moviesDF = pandas.read_csv(movies_csv).drop_duplicates(subset=["primaryTitle"], keep="first")
    actorsDF = pandas.read_csv(actors_csv).drop_duplicates(subset=["primaryName"], keep="first")
    charactersDF = pandas.read_csv(characters_csv).drop_duplicates(subset=["characters"], keep="first")
    ...
    for row in moviesDF.itertuples(name="row"):
        movie_title = row.primaryTitle.replace(" ", "_")
...
'''
    for row in charactersDF.itertuples(name="row"):
        character_name = row.characters.replace('["', '').replace('"]', '').replace(" ", "_")
...
'''
    ...
    for row in actorsDF.itertuples(name="row"):
        actor_code = row.nconst
        actor_name = row.primaryName.replace(" ", "_")
...
if __name__ == "__main__":
    writeOntologyInTTL("imdb_cinema_info.ttl", "movies.csv", "actors.csv", "principals.csv", fileBase, lastLine, ttl_url)